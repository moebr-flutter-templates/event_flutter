...
class _EATicketDetailScreenState extends State<EATicketDetailScreen> {
  ...
  void initState() {
    ...
    ticketTabs.add(Tab(text: "${DateFormat('EEE').format(now)} ${DateFormat('d MMM').format(now)}".toUpperCase()));
    ticketTabs.add(Tab(text: "${DateFormat('EEE').format(DateTime.now().add(Duration(days: 1)))} "
        "${DateFormat('d MMM').format(DateTime.now().add(Duration(days: 1)))}".toUpperCase()
    ));
  }
